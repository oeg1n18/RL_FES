...
class tf_hand_env(tf_agents.environments.py_environment.PyEnvironment, ABC):

    def __init__(self):
        super().__init__()
        self._action_spec = array_spec.BoundedArraySpec(
            shape=(1,), dtype=np.int32, minimum=0, maximum=61, name='action')
        self._observation_spec = array_spec.BoundedArraySpec(
            shape=(5,), dtype=np.float32, minimum=[0, 0, 0, 0, 0], name='observation')
    ...
    def _step(self, action):
        ...
        # take action
        if action < n_AC:
            chan1 = perms[action[0]][0]
            chan2 = perms[action[0]][1]
        elif action == n_AC:
            amp1 += 0.01
